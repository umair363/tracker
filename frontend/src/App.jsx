...
export default function App() {
  ...
  const loadAll = useCallback(async () => {
    setLoading(true);
    try {
      const [don, exp, set] = await Promise.all([
        api.getDonations(),
        api.getExpenses(),
        api.getSettings(),
      ]);
  ...
  const addDonation = async (form) => {
    ...
    } catch (e) { notify("Error saving", "error"); return; }
  ...
  const editDonation = async (form) => {
    ...
    try {
      await api.updateDonation(prev.id, { donor_name: form.donorName||null, amount: Number(form.amount), method: form.method, reference: form.reference, notes: form.notes||null, edit_log: log });
    } catch (e) { notify("Error updating", "error"); return; }
  ...
  const addExpense = async (form) => {
    ...
    if (form.receiptFile) {
      try {
        const res = await api.uploadReceipt(form.receiptFile);
        receipt_url = res.url;
  ...
  const editExpense = async (form) => {
    ...
    try {
      await api.updateExpense(prev.id, { description: form.description, amount: Number(form.amount), category: form.category, notes: form.notes||null, line_items: form.line_items||null, receipt_url, edit_log: log });
    } catch (e) { notify("Error updating", "error"); return; }
